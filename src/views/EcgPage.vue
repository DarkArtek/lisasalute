<template>
  <div class="p-4 pb-24">
    <h1 class="text-2xl font-bold mb-6 text-center text-purple-700 dark:text-purple-400">
    ...
    <div v-if="!loading && ecgs.length === 0" class="text-center text-gray-500 py-10">
      <p>Non hai ancora caricato nessun tracciato ECG.</p>
      <p class="text-sm mt-2">Puoi caricarne uno dalla Chat cliccando sulla graffetta.</p>
    </div>
...
// Funzione che genera URL firmati per una lista di record
const generateSignedUrls = async (records) => {
  ...
  // Mappiamo ogni record a una promessa per ottenere il suo URL
  const promises = records.map(async (record) => {
    ...
    const { data, error } = await supabase
      .storage
      .from('ecg_uploads')
      .createSignedUrl(record.ecg_storage_path, 3600); // URL valido per 1 ora

    if (error) {
      console.error("Errore URL firmato:", error);
...
// Carica i dati e poi genera gli URL
onMounted(async () => {
  await fetchEcgs();
});
